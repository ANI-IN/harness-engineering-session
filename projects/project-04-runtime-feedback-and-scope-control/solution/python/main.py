"""kb v4: runtime feedback and scope control, Python track.

Project 04's delta over project 03 (see SPEC.md "Delta from project 03"):
structured logging into the data directory with a `logs` surface,
corrupt-index detection (empty chunks a past buggy chunker left behind)
...
derived state), a WIP=1 check in the workspace doctor, and a server that
refuses non-GET methods. Logging is deterministic by course rule: entries
carry a per-file sequence number, never a wall clock (a real deployment
would add timestamps at the seam).
"""
