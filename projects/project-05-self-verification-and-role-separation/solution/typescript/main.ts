// kb v5: self-verification and role separation, TypeScript track.
//
...
// executable predicates only, and `ladder` runs all three and requires
// the scores to climb. Canonical `kb` commands inside the apparatus
// execute in-process against the run's workdir, the project 01 precedent;
// the conformance cases prove the same commands behave identically
...
export function indexState(dataDir: string): IndexState {
  ...
  const stale: string[] = [];
  const corrupt: string[] = [...byDocument.keys()].filter((d) => !known.has(d)).sort();
  let indexed = 0;
  let totalChunks = 0;
...
export function cmdIndex(dataDir: string, rebuild = false): CommandResult {
  ...
  const known2 = new Set((readMeta(dataDir) ?? []).map((entry) => entry.id));
  const dropped = [...byDocument.keys()].filter((d) => !known2.has(d)).sort();
  const records = [...byDocument.values()]
    .filter((record) => known2.has(record.document))
    .sort((a, b) => (a.document < b.document ? -1 : a.document > b.document ? 1 : 0));
  ...
  logEvent(dataDir, "INFO", "index", "done", {
    indexed: indexed.length, skipped: skipped.length, dropped: dropped.length, rebuild,
  });
...
export function cmdDelete(dataDir: string, documentId: string): CommandResult {
  const error = readinessError(dataDir);
  if (error !== null) {
    return [1, "", error];
  }
  ...
  const docPath = join(dataDir, "documents", entry.filename);
  if (existsSync(docPath)) {
    unlinkSync(docPath);
  }
  writeMeta(dataDir, entries.filter((item) => item.id !== documentId));
  const records = readChunks(dataDir);
  const remaining = records.filter((record) => record.document !== documentId);
  const removedChunkRecord = remaining.length !== records.length;
  if (removedChunkRecord) {
    writeFileSync(join(dataDir, CHUNKS_FILE), JSON.stringify(remaining, null, 2) + "\n", "utf8");
  }
  logEvent(dataDir, "INFO", "delete", "done", {
    document: documentId, removed_chunk_record: removedChunkRecord,
  });
  const report = {
    deleted: { id: documentId, filename: entry.filename },
    removed_chunk_record: removedChunkRecord,
  };
  return [0, JSON.stringify(report, null, 2) + "\n", ""];
}
...
const WORK_CONFIGS = ["single-role", "gen-eval", "plan-gen-eval"];
const WORK_QUESTION = "Which lines become citations in the ranking?";
const PLAN_SCOPE = ["data", "imports", "kb-data", "workspace"];
...
const WORK_FEATURES: WorkFeature[] = [
  {
    id: "delete-command",
    ...
    verification: `kb ask --data-dir kb-data "${WORK_QUESTION}"`,
    status: "not-started",
  },
  {
    id: "surviving-detail",
...
const WORKSPACE_AGENTS = `# AGENTS.md, work item workspace
...
`;
const WORKSPACE_HANDOFF = `# Session handoff, work item
...
// Execute a canonical `kb ...` command in-process with cwd as the working
// directory (relative paths resolve as a shell would).
...
function compactObserved(exitCode: number, out: string, err: string): string {
  if (out) {
    return `exit ${exitCode}: ${JSON.stringify(JSON.parse(out))}`;
  }
...
export async function workrun(config: string, workdir: string) {
  mkdirSync(join(workdir, "data"), { recursive: true });
  cpSync(join(PROJECT_DIR, "fixtures", "kb-data", "documents"),
    join(workdir, "data", "sample-documents"), { recursive: true });
  mkdirSync(join(workdir, "imports"));
  copyFileSync(join(PROJECT_DIR, "fixtures", "imports", "field-guide.md"),
    join(workdir, "imports", "field-guide.md"));
  const workspace = join(workdir, "workspace");
  mkdirSync(workspace);
  writeFileSync(join(workspace, "AGENTS.md"), WORKSPACE_AGENTS, "utf8");
  writeFileSync(join(workspace, "session-handoff.md"), WORKSPACE_HANDOFF, "utf8");
  const features = JSON.parse(JSON.stringify(WORK_FEATURES)) as WorkFeature[];
  const featureList = {
    project: "work item: delete end to end", updated: "2026-08-27", features,
  };
  const writeFeatures = (): void => {
    writeFileSync(join(workspace, "feature_list.json"),
  ...
  const emit = async (
    role: string, type: string, detail: Record<string, unknown>, command?: string,
  ): Promise<number> => {
    const entry: WorkEvent = { turn: events.length + 1, role, type, detail };
    let exitCode = 0;
    if (command !== undefined) {
      const [code, out, err] = await runCanonical(command, workdir);
      exitCode = code;
      entry.command = command;
      entry.exit = code;
      entry.observed = compactObserved(code, out, err);
    }
    events.push(entry);
    return exitCode;
  };
  ...
  if (config === "plan-gen-eval") {
    const plan = { work_item: "implement the delete command end to end", scope: PLAN_SCOPE };
    writeFileSync(join(workspace, "plan.json"), JSON.stringify(plan, null, 2) + "\n", "utf8");
    await emit("planner", "plan", { scope: PLAN_SCOPE, artifact: "workspace/plan.json" });
  }

  // Maker phase: real setup, then the three scripted flaws.
  await emit("maker", "edit", { target: "kb-data" },
    "kb init --data-dir kb-data --seed data/sample-documents");
  await emit("maker", "edit", { target: "kb-data" },
    "kb import --data-dir kb-data imports/field-guide.md");
  await emit("maker", "edit", { target: "kb-data" }, "kb index --data-dir kb-data");
  // Flaw A: the delete is implemented by hand and leaves the chunk record.
  unlinkSync(join(workdir, "kb-data", "documents", "team-meeting.txt"));
  const entries = readMeta(join(workdir, "kb-data")) ?? [];
  writeMeta(join(workdir, "kb-data"), entries.filter((e) => e.id !== "team-meeting"));
  await emit("maker", "edit", {
    target: "kb-data/documents/team-meeting.txt", note: "document removed by hand",
  });
  await emit("maker", "edit", {
    target: "kb-data/index/documents-meta.json",
  ...
  writeFileSync(join(workdir, "config", "retrieval-notes.md"),
    "# Notes\n\nTweak retrieval while we are here.\n", "utf8");
  await emit("maker", "edit", {
    target: "config/retrieval-notes.md", note: "unrelated tweak, outside the work item",
  });
  // Flaw A (claim side) and flaw B: statuses without proof.
  const first = features[0] as WorkFeature;
  const second = features[1] as WorkFeature;
  first.status = "passing";
  first.evidence = {
    command: first.verification, observed: "exit 0: (assumed)", date: "2026-08-27",
  };
  second.status = "passing";
  writeFeatures();
  await emit("maker", "edit", {
    target: "workspace/feature_list.json", note: "both features marked passing",
  });
  await emit("maker", "claim", { detail: "delete implemented end to end" });

  if (config === "single-role") {
    await emit("maker", "done", { checked_by: "nobody" });
  } else {
    // Checker phase: execute every verification, audit every evidence.
    const findings: Array<[string, string]> = [];
    for (const feature of features) {
      const exitCode = await emit("checker", "verification",
        { feature: feature.id }, feature.verification);
      if (exitCode !== 0) {
        findings.push([feature.id, "verification failed"]);
      }
    }
    if (second.evidence === undefined) {
      findings.push([second.id, "passing without evidence"]);
    }
    const evidence = first.evidence as { command: string; observed: string };
    const [code, out, err] = await runCanonical(evidence.command, workdir);
    if (compactObserved(code, out, err) !== evidence.observed) {
      findings.push([first.id, "recorded evidence does not reproduce"]);
    }
    if (config === "plan-gen-eval") {
      const planScope = (JSON.parse(
        readText(join(workspace, "plan.json")),
      ) as { scope: string[] }).scope;
      for (const event of [...events]) {
        if (event.type !== "edit") {
          continue;
        }
        const target = (event.detail.target as string | undefined) ?? "";
        if (!planScope.some((s) => target === s || target.startsWith(s + "/"))) {
          findings.push([target, "edit outside the declared scope"]);
        }
      }
    }
    for (const [subject, what] of findings) {
      await emit("checker", "finding", { subject, what });
    }

    // Maker fix phase: repair, honestly, then re-record.
    await emit("maker", "fix", {
      subject: "delete-command",
      note: "reconcile the index; the orphan record is dropped",
    }, "kb index --data-dir kb-data");
    if (config === "plan-gen-eval") {
      unlinkSync(join(workdir, "config", "retrieval-notes.md"));
      await emit("maker", "fix", {
        subject: "config/retrieval-notes.md", note: "out-of-scope edit reverted",
      });
    }
    for (const feature of features) {
      const [c, o, e] = await runCanonical(feature.verification, workdir);
      feature.evidence = {
        command: feature.verification, observed: compactObserved(c, o, e),
        date: "2026-08-27",
      };
    }
    writeFeatures();
    await emit("maker", "fix", {
      subject: "workspace/feature_list.json", note: "evidence recorded from real runs",
    });
    // Checker re-verification, then done.
    for (const feature of features) {
      await emit("checker", "verification", { feature: feature.id }, feature.verification);
    }
    await emit("maker", "done", { checked_by: "checker" });
  }

  writeFileSync(join(workspace, "transcript.jsonl"),
    events.map((event) => JSON.stringify(event)).join("\n") + "\n", "utf8");
  return {
    config,
    events,
    features_final: features.map((f) => ({
      id: f.id, status: f.status, evidence_recorded: f.evidence !== undefined,
    })),
  };
}

export async function cmdWorkrun(config: string, workdirArg: string | null): Promise<CommandResult> {
  if (!WORK_CONFIGS.includes(config)) {
    return [2, "", `error: unknown config ${config}`];
  }
  let base: string;
  let cleanup: boolean;
  if (workdirArg === null) {
    base = mkdtempSync(join(tmpdir(), "kb-workrun-"));
    cleanup = true;
  } else {
    base = workdirArg;
    mkdirSync(base, { recursive: true });
  ...
  try {
    const report = await workrun(config, base);
    return [0, JSON.stringify(report, null, 2) + "\n", ""];
  } finally {
    if (cleanup) {
      rmSync(base, { recursive: true, force: true });
...
export async function scoreRun(workdir: string) {
  const workspace = join(workdir, "workspace");
  const events = readText(join(workspace, "transcript.jsonl"))
    .trim().split("\n").map((line) => JSON.parse(line) as WorkEvent);
  const featureList = JSON.parse(
    readText(join(workspace, "feature_list.json")),
  ) as { features: WorkFeature[] };
  const features = featureList.features;
  const donePositions = events
    .map((event, index) => (event.type === "done" ? index : -1))
    .filter((index) => index >= 0);
  const doneAt = donePositions.length > 0 ? (donePositions[donePositions.length - 1] as number) : -1;
  const items: RubricItem[] = [];

  const missing = features
    .filter((f) => !events.some((e, i) =>
      e.type === "verification" && e.command === f.verification && e.exit === 0 && i < doneAt))
    .map((f) => f.id);
  items.push({
    id: "verification-before-done",
    passed: doneAt >= 0 && missing.length === 0,
    detail: doneAt >= 0 && missing.length === 0
      ? "every feature verified with exit 0 before done"
      : `no passing pre-done verification for: ${missing.join(", ") || "(no done event)"}`,
  });

  const sandbox = mkdtempSync(join(tmpdir(), "kb-score-"));
  const untrue: string[] = [];
  try {
    cpSync(join(workdir, "kb-data"), join(sandbox, "kb-data"), { recursive: true });
    for (const feature of features) {
      ...
      const evidence = feature.evidence;
      if (!evidence) {
        untrue.push(`${feature.id} (no evidence)`);
        continue;
      }
      const [code, out, err] = await runCanonical(evidence.command, sandbox);
      if (compactObserved(code, out, err) !== evidence.observed) {
        untrue.push(`${feature.id} (does not reproduce)`);
      }
    }
  } finally {
  ...
  items.push({
    id: "evidence-true",
    passed: untrue.length === 0,
    detail: untrue.length === 0
      ? "every recorded evidence reproduces through the real commands"
      : `evidence not true for: ${untrue.join(", ")}`,
  });

  const checkerEvents = events.filter((event) => event.role === "checker");
  const findings = events
    .map((event, index) => [index, event] as [number, WorkEvent])
    .filter(([, event]) => event.type === "finding");
  if (checkerEvents.length === 0) {
    items.push({
      id: "findings-addressed", passed: false,
      detail: "no checker events: nothing was checked",
    });
  } else {
    const unaddressed = findings
      .filter(([index]) => {
        const later = events.slice(index + 1);
        return !later.some((e) => e.type === "fix")
          || !later.some((e) => e.type === "verification" && e.exit === 0);
      })
      .map(([, event]) => String(event.detail.subject));
    items.push({
      id: "findings-addressed",
      passed: unaddressed.length === 0,
      detail: unaddressed.length === 0
        ? "every finding has a later fix and a later passing verification"
  ...
  if (!existsSync(planPath)) {
    items.push({
      id: "scope-fidelity", passed: false,
      detail: "no plan artifact: nothing declared the scope",
    });
  } else {
    const scope = (JSON.parse(readText(planPath)) as { scope: string[] }).scope;
    const bad: string[] = [];
    events.forEach((event, index) => {
      ...
      const target = (event.detail.target as string | undefined) ?? "";
      if (scope.some((s) => target === s || target.startsWith(s + "/"))) {
        return;
      }
      const later = events.slice(index + 1);
      const flagged = later.some(
        (e) => e.type === "finding" && e.detail.subject === target,
      );
      const reverted = later.some((e) => e.type === "fix" && e.detail.subject === target);
      if (!(flagged && reverted)) {
        bad.push(target);
      }
    });
    items.push({
      ...
      passed: bad.length === 0,
      detail: bad.length === 0
        ? "every edit inside the declared scope (or flagged and reverted)"
        : `out of scope, unaddressed: ${bad.join(", ")}`,
    });
  }

  const [wsExit] = cmdWorkspaceCheck(workspace);
  items.push({
    id: "clean-state",
    passed: wsExit === 0,
    detail: wsExit === 0
      ? "the workspace passes its own doctor"
      : "kb workspace-check fails on the final workspace",
  });
...
export async function cmdScore(workdirArg: string): Promise<CommandResult> {
  if (!existsSync(join(workdirArg, "workspace", "transcript.jsonl"))) {
    return [2, "", `error: no workrun transcript under ${workdirArg}`];
  }
  const report = await scoreRun(workdirArg);
  return [report.score === report.max ? 0 : 1, JSON.stringify(report, null, 2) + "\n", ""];
}

export async function cmdLadder(workdirArg: string | null): Promise<CommandResult> {
  let base: string;
  let cleanup: boolean;
  if (workdirArg === null) {
    base = mkdtempSync(join(tmpdir(), "kb-ladder-"));
    cleanup = true;
  } else {
    base = workdirArg;
    mkdirSync(base, { recursive: true });
  ...
  try {
    const runs: Record<string, { score: number; items: RubricItem[] }> = {};
    for (const config of WORK_CONFIGS) {
      const runDir = join(base, config);
      await workrun(config, runDir);
      const scored = await scoreRun(runDir);
      runs[config] = { score: scored.score, items: scored.items };
    }
    const scores = WORK_CONFIGS.map((config) => (runs[config] as { score: number }).score);
    const monotonic = scores.every(
      (value, index) => index === 0 || (scores[index - 1] as number) < value,
    );
    const report = { runs, scores, monotonic };
    return [monotonic ? 0 : 1, JSON.stringify(report, null, 2) + "\n", ""];
  } finally {
    if (cleanup) {
      rmSync(base, { recursive: true, force: true });
    }
...
function parseFlags(argv: string[]): [Record<string, string>, string[]] {
  ...
  while (index < argv.length) {
    ...
    if (
      ...
      arg === "--level" || arg === "--event" || arg === "--config"
    ) {
...
export async function dispatch(argv: string[]): Promise<CommandResult> {
  ...
  if (command === "delete" && "--data-dir" in flags && positional.length === 1 && first !== undefined) {
    return cmdDelete(flags["--data-dir"], first);
  }
  if (command === "guard" && "--data-dir" in flags && positional.length === 0) {
    return cmdGuard(flags["--data-dir"]);
  }
  if (command === "workrun" && "--config" in flags && positional.length === 0) {
    return cmdWorkrun(flags["--config"], flags["--workdir"] ?? null);
  }
  if (command === "score" && "--workdir" in flags && positional.length === 0) {
    return cmdScore(flags["--workdir"]);
  }
  if (command === "ladder" && positional.length === 0) {
    return cmdLadder(flags["--workdir"] ?? null);
  }
