#!/usr/bin/env bash
# project-05 verify: run the kb v5 app, the role-separation ladder, and the carried proofs through the
...
echo "verify(project-05 self-verification-and-role-separation): stack=${STACK}"
...
if uv run --project "$REPO_ROOT" python "$REPO_ROOT/tools/conformance/runner.py" \
  ...
  echo "verify: FAIL: the starter already passes the v5 cases; it must be a genuine starting point" >&2
  exit 1
fi
echo "verify: starter stage fails the v5 cases as intended (genuine starting point)"

if [ "${HARNESS_SKIP_UNIT_CONFORMANCE:-0}" != "1" ]; then
  if [ "$STACK" = "python" ] || [ "$STACK" = "both" ]; then
    (cd "$REPO_ROOT" && uv run pytest projects/project-05-self-verification-and-role-separation -q)
  fi
  if [ "$STACK" = "typescript" ] || [ "$STACK" = "both" ]; then
    ...
    (cd "$REPO_ROOT" && "$NODE20_BIN/pnpm" exec vitest run --silent=true \
      projects/project-05-self-verification-and-role-separation)
  fi
fi
