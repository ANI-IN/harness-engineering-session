...
- [Lecture 09](../lecture-09-why-agents-declare-victory-too-early/): why a
...
- **A node sees only what its graph entry lists.** Each node declares the
  ...
  independence lecture 09 argues for is enforced by the executor rather
...
```sh
L=lectures/lecture-14-graph-engineering
...
<!-- generated-block: uv run python lectures/lecture-14-graph-engineering/code/python/main.py run lectures/lecture-14-graph-engineering/code/fixtures/graphs/graph-declared.json lectures/lecture-14-graph-engineering/code/fixtures/workspaces/workspace-conflicted -->
```json
{
...
<!-- generated-block: uv run python lectures/lecture-14-graph-engineering/code/python/main.py run lectures/lecture-14-graph-engineering/code/fixtures/graphs/graph-misrouted.json lectures/lecture-14-graph-engineering/code/fixtures/workspaces/workspace-conflicted | uv run python -c "import json,sys; r=json.load(sys.stdin); print(json.dumps({'path': r['path'], 'routing': r['routing'], 'final_state': {k: r['final_state'][k] for k in ('review', 'committed', 'rolled_back')}, 'workspace_after': r['workspace_after'], 'matches_opening_state': r['matches_opening_state'], 'verdict': r['verdict']}, indent=2))" -->
```json
{
  "path": [
    "plan",
