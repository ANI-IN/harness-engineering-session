// router-validator exercise, TypeScript starter.
//
// All four checks run, but three are naive first drafts with a realistic
// mistake each (see SPEC.md "Starter state"): routes-resolve trusts that
// the route line exists instead of the target file, hard-in-entry scans
// only the entry file, and no-duplicates compares whole lines instead of
// rule text. Fix the three per SPEC.md. Run
// ../../verify.sh --stack=typescript until it exits 0. entry-length is
// already correct.
...
function checkNoDuplicates(tree: string): Violation[] {
  ...
  for (const [name, lines] of treeFiles(tree)) {
    lines.forEach((line, index) => {
      ...
      // Naive draft: comparing the whole line lets the same rule text hide
      // behind a different topic tag. Exercise: compare the rule TEXT (the
      // part after the tag).
      const text = line;
