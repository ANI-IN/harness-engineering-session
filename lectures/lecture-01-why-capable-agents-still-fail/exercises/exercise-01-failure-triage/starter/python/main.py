"""failure-triage exercise, Python starter.

Two of the five attribution rules are implemented (instructions, tools).
Your task: implement the remaining three in attribute_event, exactly as
...
Run ../../verify.sh --stack=python until it exits 0. Everything outside
attribute_event already works; you should not need to change it.
"""
...
def attribute_event(event: dict, prior: list[dict]) -> tuple[str, str] | None:
    ...
    #   "pass" in the same run; `prior` holds the earlier events.
    return None
