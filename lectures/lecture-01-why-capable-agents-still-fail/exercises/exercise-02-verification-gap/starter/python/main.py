"""verification-gap exercise, Python starter.

The report plumbing works; the classification logic does not. Your task:
in classify(), decide whether a run's FIRST claim was backed by an earlier
passing verification, per ../../SPEC.md. Run ../../verify.sh --stack=python
until it exits 0. You should not need to change anything else.
"""
...
def classify(events: list[dict]) -> dict:
    ...
    # (index < first_claim_index) is a verification with result "pass".
