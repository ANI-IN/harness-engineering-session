...
case "$TARGET" in
  starter|solution)
    echo "verify(exercise-02-verification-gap): stack=${STACK} target=${TARGET}"
    if run_stage "$TARGET"; then
      ...
      if [ "$TARGET" = "starter" ]; then
        cat <<'MSG'
The starter is expected to fail until you implement the before-claim verification check
in classify() in starter/<your stack>/main.(py|ts).
...
MSG
    ...
    ;;
  ci)
    echo "verify(exercise-02-verification-gap): stack=${STACK} target=ci"
    starter_log="$(mktemp)"
