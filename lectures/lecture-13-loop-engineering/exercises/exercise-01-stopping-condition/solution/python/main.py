...
the condition reads is the checker's verdict over every criterion, because
the maker's report covers only the step the maker just took.
"""
...
def reached_the_goal(record: dict) -> bool:
    ...
    The checker grades every criterion of the goal and is not the party
    that did the work, so its verdict is the one that can end the loop.
    """
