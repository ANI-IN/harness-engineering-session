...
// Replay the journal so the most recent write is undone first, and report
// the rows in journal order whatever order they were attempted in.
function rollback(files: Files, journal: Journal): Row[] {
  const operations = journal.operations;
  const rows: Row[] = new Array<Row>(operations.length);
