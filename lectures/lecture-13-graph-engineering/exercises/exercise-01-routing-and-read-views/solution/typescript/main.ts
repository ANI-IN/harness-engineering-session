...
//   write the routed key on the way to that node. The last writer is the
//   one whose value the router sees, so writer attribution is walk order,
//   not the order the nodes happen to appear in the graph file.
...
// The node whose write of `key` the router at `at` reads. Every node on the
// way to `at` may write the key, and each write overwrites the one before
// it, so the value the router sees belongs to the LAST writer in walk
// order, not the first one written down.
function decidingWriter(
  ...
  let writer = NONE;
  for (const name of order.slice(0, order.indexOf(at) + 1)) {
    const node = byName.get(name) as GraphNode;
    if (node.writes.includes(key)) writer = name;
  }
  return writer;
}
...
function decisionsOf(order: string[], byName: Map<string, GraphNode>): Decision[] {
  const rows: Decision[] = [];
  for (const name of order) {
    const router = (byName.get(name) as GraphNode).router;
    if (router === undefined) continue;
    const writer = decidingWriter(byName, order, name, router.key);
...
export function report(graph: Graph) {
  ...
  const decisions = decisionsOf(order, byName);
