...
* A router is independent when the node it sits on is the LAST node to
  write the routed key on the way to that node. The last writer is the one
  whose value the router sees, so writer attribution is walk order, not the
  order the nodes happen to appear in the graph file.
...
def deciding_writer(by_name: dict[str, dict], order: list[str], at: str, key: str) -> str:
    """The node whose write of `key` the router at `at` reads.

    Every node on the way to `at` may write the key, and each write
    overwrites the one before it, so the value the router sees belongs to
    the LAST writer in walk order, not the first one written down.
    """
    writer = NONE
    for name in order[: order.index(at) + 1]:
        if key in by_name[name]["writes"]:
            writer = name
    return writer
...
def decisions_of(order: list[str], by_name: dict[str, dict]) -> list[dict]:
    ...
    for name in order:
        ...
        if router is None:
            continue
        key = router["key"]
        writer = deciding_writer(by_name, order, name, key)
        rows.append(
...
def report(graph: dict) -> dict:
    ...
    decisions = decisions_of(order, by_name)
