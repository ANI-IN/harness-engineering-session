"""memo-migrator exercise, Python starter.
...
mapping in migrate() per SPEC.md. Run ../../verify.sh --stack=python until
it exits 0.
"""
...
def migrate(scope: dict, memo_text: str) -> tuple[dict | None, str | None]:
    ...
    for feature in scope["features"]:
        ...
        if feature["id"] in claims:
            # Naive draft: the memo says this one is done, so record it as
            # done. Nothing was run to show that, and the dialect has a
            # status for "claimed, unverified"; it is not this one.
