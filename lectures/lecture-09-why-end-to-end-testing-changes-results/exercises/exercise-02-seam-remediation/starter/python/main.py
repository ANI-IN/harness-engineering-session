"""seam-remediation exercise, Python starter.

The assembled run, the failure record, the producer lookup, and the report
are complete: `what` and `why` already read correctly. One naive decision
remains (see SPEC.md "Starter state"): `fix_line` writes the instruction
against the component that raised the objection. Rewrite it so the
instruction lands on the producing side of the seam, then run
...
def fix_line(failure: dict) -> str:
    """Naive draft: the failure was raised in `stage`, so `stage` is where
    the change goes; making the objection stop is the shortest route to a
    green run. Exercise: the objecting side states the contract, and the
    value that broke it arrived from `producer`; see SPEC.md."""
    if failure["kind"] == "missing":
        return f"change {failure['stage']} to accept a record without {failure['field']}"
    if failure["kind"] == "prefix":
        return f"change {failure['stage']} to accept {failure['field']}={failure['value']}"
    return f"change {failure['stage']} to expect {failure['field']}={failure['value']}"
