// claim-audit exercise, TypeScript starter.
//
...
// --stack=typescript until it exits 0.
...
export function audit(workspace: string, claim: Claim) {
  ...
  for (const claimed of claim.checks) {
    ...
    if (claimed.basis === "executed") {
      // Naive draft: the session already ran this check and recorded its
      // output, so re-running it would only repeat work. Exercise: the
      // record describes the past; re-execute regardless of basis.
      actual = claimed.status;
      detail = claimed.evidence;
    } else {
      const [passed, freshDetail] = executeCheck(workspace, check);
