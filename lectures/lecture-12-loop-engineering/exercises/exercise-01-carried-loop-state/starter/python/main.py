...
that attempt back into the state before the next round begins.
...
This draft reads the carried state up to the index of the round it stopped
on, since `rounds_done` counts the rounds that ran.
"""
...
def attempted_criteria(state: dict) -> list[str]:
    ...
    `rounds_done` is the number of the last round that ran, so the entries
    up to that round's index are the ones the loop is finished with.
    """
    return [entry["criterion"] for entry in state["attempted"][: state["rounds_done"] - 1]]
