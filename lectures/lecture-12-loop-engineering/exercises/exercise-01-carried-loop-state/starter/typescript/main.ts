...
// stop reasons.
//
// This draft reads the carried state up to the index of the round it
// stopped on, since `rounds_done` counts the rounds that ran.
...
// `rounds_done` is the number of the last round that ran, so the entries up
// to that round's index are the ones the loop is finished with.
function attemptedCriteria(state: LoopState): string[] {
  return state.attempted.slice(0, state.rounds_done - 1).map((entry) => entry.criterion);
}
