// knowledge-gap-report exercise, TypeScript starter.
//
// The report plumbing works; the visibility rule does not. inRepo() is a
// naive first draft that counts any location MENTIONING "repo" as in-repo,
// so a Confluence page about repo guidelines counts as visible to the
// agent. Fix inRepo() per SPEC.md. Run ../../verify.sh --stack=typescript
// until it exits 0.
...
export function inRepo(location: string): boolean {
  ...
  return location.toLowerCase().includes("repo");
}
