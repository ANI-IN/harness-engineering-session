...
## The whole module in one block

Everything here is verified by executable checks rather than asserted. These
counts are regenerated by the build, so they cannot drift from the tree:
...
Every one of those README commands, including the ones in this file, is
executed literally by the build. A command printed here is a command that
runs.
...
- [ ] Do not clone this into iCloud Drive, Dropbox or OneDrive. A synced
      folder races the working tree; see [Troubleshooting](#troubleshooting).
...
- **Gates fail intermittently, or empty directories appear.** You are almost
  certainly running inside a synced folder. iCloud Drive, Dropbox and
  OneDrive race the working tree, recreate directories the gates have just
  removed, and leave numbered conflict copies such as `kb-data 2`. Move the
  clone to an unsynced path such as `~/src`.
