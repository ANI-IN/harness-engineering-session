...
| --- | --- | --- |
| 01 — Why capable agents still fail | Capability ≠ reliable execution; failures live in five subsystems | Project 01 |
| 02 — What a harness actually is | Instructions, tools, environment, state, feedback — as one system | Project 01 |
...
**Node.js 20 LTS** with pnpm (via corepack — ships with Node). You only need
the toolchain for the track you choose; `make setup` installs both.
...
tools/          conformance runner + structure/link/mermaid linters
```
...
make lint             # ruff + eslint + markdownlint + shellcheck
