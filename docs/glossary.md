...
(entry file ≈ 100 lines is this course's working heuristic; the split rule
matters more than the number).

**`CLAUDE.md`** — the same contract voiced for Claude Code, which loads this
file automatically. One contract, two entry files.

**`init.sh`** — the initialization script run at session start: installs
dependencies, verifies the environment, prints the next command. Initialization
is a phase of its own, not something mixed into feature work.
...
`not-started → in-progress → passing`, with `blocked` reachable from
...
check passed and the evidence (command + result) is recorded. An agent's
...
The checker needs independent signals — tests, logs, rubrics — not the
maker's self-report.
...
status claim (e.g. `verify.sh exited 0 on 2026-08-27`). Prose about the code
is not evidence.
...
project: starter runs but fails `verify.sh` for the intended reason;
